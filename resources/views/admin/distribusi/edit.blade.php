...
<div class="bg-white rounded-2xl shadow-xl p-8 hover:shadow-2xl transition-shadow duration-300">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-xl font-bold text-emerald-800">Edit Distribusi Zakat</h2>
    </div>
    ...
    <form method="POST" action="{{ route('admin.distribusi.update', $distribusi->id) }}" class="space-y-6">
        ...
        @unless(in_array($slug, ['uang-tunai', 'beasiswa']))
            <div>
                ...
                <input type="number" name="jumlah" value="{{ old('jumlah', $distribusi->jumlah) }}" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-300 bg-white shadow-sm" placeholder="Masukkan jumlah" />
                @error('jumlah') 
                    <span class="text-sm text-red-600 mt-1 flex items-center gap-1">
                        <i class="fas fa-exclamation-triangle"></i> {{ $message }}
                    </span> 
        ...
        <div>
            ...
            <input type="date" name="tanggal" value="{{ old('tanggal', $distribusi->tanggal) }}" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-300 bg-white shadow-sm" />
            @error('tanggal') 
                <span class="text-sm text-red-600 mt-1 flex items-center gap-1">
                    <i class="fas fa-exclamation-triangle"></i> {{ $message }}
                </span> 
