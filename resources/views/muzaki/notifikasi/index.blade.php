...
<h2 class="text-xl font-bold text-emerald-800 mb-6 flex items-center">
    <i class="fas fa-bell text-emerald-600 mr-2"></i> Notifikasi Saya
</h2>

<ul class="space-y-3">
    @forelse($notifikasi as $notif)
        <li class="p-4 bg-white/80 backdrop-blur-sm rounded-lg shadow-sm hover:shadow-md transition flex items-start justify-between border border-emerald-100">
            <div class="flex items-start space-x-3">
                ...
                <div>
                    ...
                    <a href="{{ route('muzaki.notifikasi.show', $notif->id) }}" 
            ...
            @if(!$notif->status_baca)
                <form method="POST" action="{{ route('muzaki.notifikasi.baca', $notif->id) }}">
                    @csrf
                    <button class="text-xs font-medium text-emerald-600 hover:text-emerald-800">
                        Tandai dibaca
                    </button>
                </form>
            @else
